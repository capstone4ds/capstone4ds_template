...
age (≥20 years), gender, and race as predictors, using a retrospective
dataset from the 2013–2014 NHANES survey. NHANES employs a complex
...
Fit classical regression or machine learning models on the training set.
Obtain point estimates (e.g., coefficients, odds ratios) and confidence intervals.
(4)**Bayesian Approach**
Fit a Bayesian model using the same training set.
Obtain posterior distributions (posterior draws) for parameters to quantify uncertainty.
...
Use both frequentist and Bayesian models to simulate or assess interventions
Identifying modifiable risk factors (e.g., BMI, lifestyle factors).
Predicting how changing a factor affects outcomes (e.g., diabetes risk).
In Bayesian analysis, quantifying uncertainty in intervention effects using posterior predictive distributions.
...
## Statistical Tool 
- R, R packages and libraries to import data, perform data wrangling and analysis.
## Data source
- NHANES 2-year data is a cross-sectional weighted data
(2013-2014 year) @CenterforHealthStatistics1999. 
- Three datasets
...
**Data pre-processing and cleaning** Subsets created from the
original weighted datasets (demographics, exam, questionnaire) with
selected variables merged using ID to create a single dataframe.

1.  Response Variable: Binary, **type 2 / diagnosed diabetes**
    (excluding gestational diabetes) -“Doctor told you have diabetes?”
...
```
## Description and Visualzation of Adult (> 20 years) dataset and variables
...
## Summary of Bayesian regression under following headins - 
...
MICE was performed and below is the model fit on the imputed dataset. The summary of the odds ratio of the pooled imputed dataset is presented
...
## Bayesian Logistic Regression 
Below we perform Bayesian Logistic Regression on one of the pooled imputed adult dataset (adult_imp1)  and provide the Bayesian model and summary statistics
...
Visualization, summary and tables of the imputed adult dataset is presented here
...
Bayesian Logistic Regression and Prior 
...
Visualization of the posterior draws (pp check, ppc dens plots, ppc (mean and SD) and ppc bars )
...
- Visualization of (fitted) Predicted vs observed for bmi using point and error bars
- Visualization of (fitted) Predicted vs observed for bmi using line plot
...
Plot - Posterior predicted proportion of Diabetes vs NHANES prevalence of Diabetes in the population
