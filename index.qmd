...
3.  Posterior Odds Ratios provides interpretation of the model
    coefficients on a multiplicative scale with reference categories: NH White (race), Male
    (sex).
4.  Posterior Predictive Checks (PPC) assesses how the model reproduces observed data
...
1.  Posterior Parameter Distributions assess uncertainty and effect
...
-   Plotted density overlays (mcmc_areas) for key predictors: age, BMI,
    sex, and race categories showing distribution of
...
- A comaprative results from observed BMI (bmi) and predicted
posterior estimates (predicted_bmi) with 95%
...
Summary of observed data and centered BMI 
