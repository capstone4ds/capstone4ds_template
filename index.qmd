...
-   The shaded regions indicate the 80% credible intervals within which
...
percent_pred1_each_obs <- colMeans(pp_samples) * 100
head(percent_pred1_each_obs)



overall_percent_pred1 <- mean(pp_samples) * 100
overall_percent_pred1
...
# Extract prior draws (if not saved earlier)
pri <- prior_draws

# Put both into long format
...
all_draws <- bind_rows(pri_long, post_long)

# Plot
ggplot(all_draws, aes(x = value, fill = type, color = type)) +
  geom_density(alpha = 0.3) +
  facet_wrap(~ term, scales = "free") +
  theme_bw() +
  labs(title = "Prior vs Posterior Density",
       subtitle = "Shows shrinkage and prior influence")
```

```{r}
#| label: sd calculated

yrep <- posterior_predict(bayes_fit)

pp_sd <- apply(yrep, 1, sd)
mean(pp_sd)
...
# Survey-weighted estimates with confidence intervals
svy_est <- tidy(svy_fit, conf.int = TRUE) %>%
  filter(term %in% c("age_c", "bmi_c")) %>%
  mutate(
    term = recode(term, "age_c" = "Age", "bmi_c" = "BMI"),
    svy = paste0(
      round(estimate, 3), 
      " (", 
      round(conf.low, 3), ", ", 
...
# Bayesian estimates with credible intervals
bayes_est <- tidy(bayes_fit, conf.int = TRUE) %>%
  filter(term %in% c("age_c", "bmi_c")) %>%
  mutate(
    term = recode(term, "age_c" = "Age", "bmi_c" = "BMI"),
    bayes = paste0(
...
final_table <- left_join(svy_est, bayes_est, by = "term")
final_table
