...
and analysis. All computation is implemented in a probabilistic
programming environment using the brms interface to Stan.
...
-   Adult dataset:Three NHANES datasets are merged: demographics
    (DEMO_H), body measures (BMX_H), and the diabetes questionnaire
    (DIQ_H) in (.XPT) format, imported via Haven package in R. Variables
    of interest are selected using the original weighted datasets and ID
    to create a single adult analytic dataframe.
-   All variables were coerced to consistent numeric or factor types
    prior to merging to ensure atomic columns suitable for survey
...
-   Age and BMI are strong positive predictors of diabetes — each 1 SD
    increase in these variables substantially raises the odds.
