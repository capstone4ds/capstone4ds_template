...
-   quick weighted checks
-   effective sample size for diabetes
-   Design effect = actual variance / SRS variance
...
Two modeling frameworks were compared using identical predictors
(standardized age, BMI, sex, and race4) and the binary outcome
diabetes_dx:

(1) Survey-weighted logistic regression and and complete-case analysis\
(2) Bayesian logistic regression with weakly informative priors was
    conducted after perfroming multiple imputation (MICE) o address
    missing values.
...
### (2) Survey weighted Multiple Logistic Regression model
...
All non-White racial groups have significantly higher odds of diabetes,
highlighting persistent disparities in diabetes risk. - All predictors
are statistically significant (p \< 0.01), suggesting a robust
association across demographic and health variables.
...
Imputation performed on the survey weighted adult data (obs = 5769)
...
-   A Bayesian logistic regression model fitted on the first pooled
    imputed dataset (adult_imp1)
-   with survey weights-Normalized MEC exam weights (wt_norm) and mean
    1.00 (SD 0.79)
-   with no missing values
-   with continuous variables standardized
-   categorical variables re-leveled for reference categories
...
Family: bernoulli Links:

mu = logit

Data: adult_imp1 (Number of observations: 5592)

Draws: 4 chains, each with iter = 2000; warmup = 1000; thin = 1; total
...
**Model Estimation (Posterior draws)** - Four Markov Chain Monte Carlo
(MCMC) chains, each with 2000 iterations (50% warm-up), and an adaptive
delta of 0.95 ensured good chain convergence and reduced divergent
transitions.
...
-   All Rhat ≈ 1.00 → model convergence is excellent - ESS values are
    large (\>1000), indicating stable posterior sampling and indicate
    model reliability.
...
-   Other Hispanics have slightly higher odds (≈1.5×), but the credible
    interval overlaps 0 → uncertain evidence - Non-Hispanic Blacks have
    \~1.7× higher odds, with credible interval \>0 → significant
    association.
...
-   Plot 1: The density plots of standardized BMI and Age (imputed
    dataset) (adult_imp1) show

    -   approximately normal distributions centered near zero,
        consistent with z-score standardization

    -   confirming that both predictors were properly centered and
        scaled prior to Bayesian modeling, ensuring comparability and
        numerical stability during estimation.

(2) Plot2: Prior Distributions for Coefficients

-   Priors for regression coefficients were drawn from a Normal(0, 2.5)
    distribution,
-   The prior density plots for Age (per 1 SD) and BMI (per 1 SD)
    demonstrate symmetric bell-shaped distributions, indicating no
    strong bias toward positive or negative effects before observing
    data.
...
-   Each Trace Plots shows 4 MCMC chains (different colors) across 1000
    iterations
...
### Posterior Distribution Plots (Left Panels)
...
-   The Bayesian logistic regression model identified significant
    ...
    diabetes diagnosis.
-   Odds ratios: Bayesian posterior odds ratios (95% CrI) with reference
    group being: NH White, Male
    -   Age a strong predictor: each standardized unit increase in age
        was associated with nearly threefold higher odds of diabetes (OR
...
-   **Reference**: *Male* , *Non-Hispanic White.* Age and BMI are
    standardized (per 1 SD).\*

**Posterior Coefficients (on log-odds scale) Interpretation**

#### **Intercept:** `b_Intercept = –2.66 (SD = 0.088, LCL ≈ –2.84)`

-   Represents baseline log-odds of diabetes for: **male, NH White,
    average age, average BMI** with \~ **6.5% baseline diabetes
    probability**.
...
-   A total of 500 posterior predictive sample draws generated 5,592
    observations, producing a simulated distribution of predicted
    outcomes to assess how the model reproduces observed data and
    validate model fit, visualizations below compares the predicted and
    observed data
    -   ppc_bars() plot compared the observed diabetes outcomes with 500
        replicated datasets drawn from the posterior distribution.
    -   Density overlays for both mean and SD. There was no large
        discrepancies indicating potential misfit and good alignment
...
The plot here shows **how well the model's predicted probabilities match
the observed outcomes**, across individuals and checks model calibration
Logistic regression fit at person-level.
...
-   Points between → the model is uncertain or mispredicting.
...
-   The shaded regions indicate the 80% credible intervals, representing
    the range within which the true parameter values are most likely to
    lie based on the posterior samples.
...
**Interpretation**: Density plots show an overlay of the prior and
posterior distributions for BMI and age
...
-   The posterior predictive proportions of diabetes closely matched the
    observed prevalence in the analytic dataset, supporting the model’s
    reliability for inference and prediction.
...
-   Posterior predictive mean (Bayesian model): 10.95%
    ...
    -   the model allows for values as low as 8.5% and as high as 12.8%.
        The predicted proportion incorporates uncertainty from Bayesian
        model, providing a more robust estimate of diabetes
        prevalence.These results suggest that approximately 1 in 10
        adults in this population may have diabetes.

-   Bayesian posterior predictions yield slightly higher prevalence
    estimates than the raw survey-weighted mean. The posterior
    predictive distribution closely matches the observed prevalence,
    suggesting that the Bayesian model is well-calibrated.
...
-   NHANES estimate falls near the lower tail of the posterior
    distribution, suggesting that the model still broadly captures the
    population prevalence, though it leans toward higher predicted
    rates.This visualization highlights the model’s ability to capture
