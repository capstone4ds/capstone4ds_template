...
# --- Step 1: Frequentist logistic regression (from m_imp) ---
freq_summary <- coef(summary(m_imp)) %>%
  as.data.frame() %>%
  rownames_to_column(var = "Predictor") %>%
  rename(Estimate = Estimate, Std.Error = `Std. Error`, z = `z value`, p_value = `Pr(>|z|)`) %>%
  mutate(
    OR = exp(Estimate),
  ...
  select(Model, Predictor, OR, Lower_CI, Upper_CI, p_value)

# --- Step 2: Bayesian logistic regression (from Diabetes_prior) ---
# Assuming you have a summary data frame like: Diabetes_prior_summary
bayes_summary <- Diabetes_prior$summary %>%
  as.data.frame() %>%
  rownames_to_column(var = "Predictor") %>%
  rename(Estimate = Estimate, Lower_CI = l.95..CI, Upper_CI = u.95..CI) %>%
  mutate(
    OR = exp(Estimate),
    Model = "Bayesian",
    p_value = NA
...
# --- Step 3: Firth penalized logistic regression (from m_firth) ---
firth_coef <- m_firth$coefficients
firth_se <- m_firth$se
firth_summary <- tibble(
  Predictor = names(firth_coef),
  Estimate = firth_coef,
  Std.Error = firth_se,
  Lower_CI = m_firth$ci.lower,
  Upper_CI = m_firth$ci.upper,
  Model = "Firth",
  p_value = m_firth$prob
) %>%
  mutate(
    OR = exp(Estimate)
  ) %>%
...
all_summary <- bind_rows(freq_summary, bayes_summary, firth_summary)

# --- Step 5: Print as nice table ---
kable(all_summary, digits = 3, caption = "Regression Results: Frequentist, Bayesian, and Firth Models")
