...
**(1) Survey-weighted logistic regression and complete-case analysis**
...
Strongest predictors: Age and BMI show the largest ORs among continuous
variables—both strongly linked to diabetes risk.
...
## Distribution of prior and posterior

(1) Plot 1: The density plots of standardized BMI and Age (imputed
...
We generates predicted outcomes for each observation using samples from
the posterior distribution of the model parameters with ndraws = 500,
samling 500 draws from the posterior to simulate predicted data to
compare these simulated outcomes with the actual observed data to see if
the model can reasonably reproduce the patterns in the data
