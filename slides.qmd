...
-   **Predict Diabetes** by early identification of **risk factors**
...
-   **Application in Targeted Interventions**:

    -   To improve estimation of disease burden and risk factors in specific subgroups or individuals by combining survey data, prior research, and posterior predictive modeling
...
-   Supports **model checking, variable selection,** and the model is flexible.

-   Better for individualized/personalized risk prediction
...
MCMC convergence: the GIBBS sampler has reached the posterior distribution — the point where the information from the prior and the data (likelihood) has been fully combined, and the chain is now drawing valid samples from that posterior.
...
-   After convergence across all mcmc draws have
    ...
    precise estimates, high uncertainty, narrow distributions, smooth and unimodal → no multimodality **confirming stable posteriors.**
...
**After seeing the data,** Posterior draws = 4000 draws (4 chains × 1000 iterations) via MCMC gives a posterior distribution of predictors

-   **Conditional distribution** of the parameter given the data.
-   **Subjective** -\> prior distribution influences the conclusions.
-   Influence of the prior **goes to zero as the sample size increases.**
-   It is the **full probability distribution** of the parameter **(mountain)**
...
**y-axis:**Density (how frequent each predicted probability occurs)\
**x-axis:** Predicted probability values (from 0 to 1).

The model can generate data that **looks like the observed data**\
...
-   **Prior distribution: wide spread (variance)** -shows a high degree of initial uncertainty or a non-informative prior

-   P**osterior distribution:**

    -   Different from the priors: **Narrower and taller**

    -   **Data has significantly updated the initial beliefs**
...
**500 draws: predicted outcomes** for each observation compared observed data vs predicted outcomes (binary outcome = 0 or 1) simulated

::: panel-tabset
#### Pred vs Obs Outcome

![](images/clipboard-2418526955.png){width="274"}\
**Fig**: a bar plot comparing counts/frequencies of each category (0 vs 1) in the observed data (y) and in the posterior predictive samples (y-rep)\

**Bars for y (observed) fall within the range of yrep (simulated):** **model predicts the overall diabetes prevalence well**

#### Pred Outcome Proportion
...
-   Model’s posterior mean diabetes prevalence = 10.95% (95% CI: 8.5%–12.8%) \> NHANES estimate of 8.9% (SE = 0.0048)

**Slightly higher** mean prevalence, but the credible interval overlaps the survey estimate = reasonable calibration.
...
#### Observed vs. Predicted Average diabetes (aross individuals)
...
-   The **Bayesian model complements frequentist approaches** by providing **stable, interpretable estimates with quantified uncertainty**.

-   The model broadly reproduces **population-level prevalence.**

-   **Normalized weights** were used to approximate survey weighting, but the **model does not fully account for stratification, clustering, or design-based variance adjustments.**

##### Interpretation

-   Minimal shrinkage and similar uncertainty - reliability and robustness

-   Estimates are pulled slightly towards the prior mean (often toward zero or toward more moderate values) compared to classical estimates.

-   The Bayesian posterior did not pull the estimates very much.

-   **The observed data was strong enough to dominate the prior.**
...
Model enables individualized diabetes risk prediction using age, BMI, sex, race can guide actionable thresholds (e.g., risk \>30%)

-   BMI is modifiable - interventions can lower risk (Sex and race remain constant)
-   Selected one participant (adult\[1, \]) with all covariates (age, BMI, sex, race)
-   using **posterior_linpred(transform = TRUE)**, we obtained predicted probabilities with the 95% credible interval to predict probability\

![](images/clipboard-2754134965.png){width="196"}

**Median predicted probability ≈ 0.25, with a 95% credible interval of 0.20–0.31, indicate a \~1 in 4 chance of diabetes.**
...
-   Computed the posterior predicted probability with 95% credible intervals for a new participant data on age, BMI, sex, and race.\
...
Reverse prediction illustrates the BMI value associated with a specific diabetes probability for a given individual
