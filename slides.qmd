...
-   Predictors: age, sex, race, BMI - Response variable: `diabetes_dx` (diagnosis: 0 = No, 1 = Yes)
...
Fit logistic regression model on imputed dataset and Pool estimates f

``` r
fit_mi \<- with(data = imp, exp = glm(diabetes_dx \~ age_c + bmi_c + sex + race, family = binomial())) pool_mi \<- pool(fit_mi)\`
```
...
-   **All predictors show positive associations with diabetes risk — age, BMI, and hypertension are key independent predictors.**
-   **The narrow confidence intervals suggest precise estimates, and since the CIs do not include zero, the effects are statistically significant**.
...
![](Prior%20vs%20Posterior%20Distributions_bmi_age.png){width="415"}
:::
