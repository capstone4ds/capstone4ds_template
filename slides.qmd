...
**After seeing the data** - Posterior distribution reflects the data, and also the regularizing pull of the prior\
...
- Probability of very large or very small parameter values is very low and **stabilizes the model** ![](images/clipboard-3802967430.png){width="259"}\| ![](images/clipboard-2442291846.png){width="259"}
:::
...
**Bayesian models assume complete data.** **Adults data stats**
...
**GIBBS SAMPLER: A specific algorithm under the MCMC category** - Produces a sequence of random vectors ![](images/clipboard-1640325740.png){width="90" height="35"}

-   Each sample **depends on the past only through the most recent one**.

-   The samples are **not just random**

-   Samples are drawn in a way that the long-term distribution of the samples matches the target posterior distribution.

-   It uses a **burn in period**.

-   The random vectors are s**equentially dependent**.
...
To evaluate we look for

-   the **convergence with no drift across iterations**

-   **well-mixed chains** without any trends with stable posterior estimates.

-   **R-hat ≈ 1** → good chain mixing, Effective sample size
...
**MCMC samples (posterior distribution/draws)**

Posterior draws = 4000 draws (4 chains × 1000 iterations) for posterior predictive checks.
...
-   The posterior distribution is the **full probability distribution** of the parameter **(mountain)**; the mean and credible interval are just summaries of that distribution, not the distribution itself.
...
**y-axis:**Density (how frequent each predicted probability occurs) **x-axis:** Predicted probability values (from 0 to 1).
...
#### Posterior Estimates and Distribution
...
-   Prior distribution: The **wide spread (variance)** - a high degree of initial uncertainty or a non-informative prior

-   Posterior distribution: are different from the priors and are **narrower and taller.** The **data has significantly updated the initial beliefs**
...
::: panel-tabset
#### Pred vs obs outcome (proportion)
...
#### Outcome Proportion
x-axis: proportion of diabetics (1s), y-axis: frequency of posterior draws with that proportion\
![](images/clipboard-122565288.png){width="216"}\
Spread shows the uncertainty in predicted prevalence\
![](images/clipboard-2142249375.png){width="246"}\
The model’s posterior mean diabetes prevalence was 10.95% (95% CI: 8.5%–12.8%), slightly higher than the NHANES estimate of 8.9% (SE = 0.0048)\
but the credible interval overlaps the survey estimate, indicating reasonable calibration.
...
**Predict BMI for estimated Diabetes**
